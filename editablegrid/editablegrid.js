...
EditableGrid.prototype.loading = false;
...
{
    if (this.loading) {
        console.log("Forbidden - loading detected.");
        return;
    }
	...
	ajaxRequest.onreadystatechange = function () {
	    if (this.readyState == 4) {
		    ...
		    if (!this.responseText) {
		        console.error("Could not load JSON from url '" + url + "'"); return false;
		    }
		    if (!self.processJSON(this.responseText)) {
		        console.error("Invalid JSON data obtained from url '" + url + "'"); return false;
		    }
...
{	
    ...
    // need relogin
	if (jsonData.status == "Error" && jsonData.errorCode == "-999" && jsonData.errorDescription == "Please Login") {
